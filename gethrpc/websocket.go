...
func DialWebsocketWithParams(ctx context.Context, endpoint, origin string, skipVerify bool, headers map[string]string, connectionTimeout, wsReadDeadline time.Duration) (*Client, error) {
	endpoint, header, err := wsClientHeaders(endpoint, origin)
	if err != nil {
		return nil, err
	}
