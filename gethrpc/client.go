...
func Dial(rawurl string, skipVerify bool, headers map[string]string, connectionTimeout, websocketReadDeadline time.Duration) (*Client, error) {
	return DialContext(context.Background(), rawurl, skipVerify, headers, connectionTimeout, websocketReadDeadline)
}
...
func DialContext(ctx context.Context, rawurl string, skipVerify bool, headers map[string]string, connectionTimeout, wsReadDeadline time.Duration) (*Client, error) {
	...
	case "http", "https":
		if skipVerify {
			return DialHTTPWithClient(rawurl, &http.Client{Transport: &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: true}}}, headers)
		}
		return DialHTTP(rawurl)
	case "ws", "wss":
		if skipVerify {
			return DialWebsocketWithParams(ctx, rawurl, "", skipVerify, headers, connectionTimeout, wsReadDeadline)
		}
