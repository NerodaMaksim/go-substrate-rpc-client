...
func Connect(url string) (Client, error) {
	...
	c, err := gethrpc.DialContext(ctx, url, false, nil, time.Duration(0), time.Duration(0))
	if err != nil {
		return nil, err
	}
	cc := client{*c, url}
...
// ConnectWithHTTPClient connects to the provided url with a custom http client
func ConnectWithHTTPClient(url string, insecureSkipVerify bool, headers map[string]string, connectionTimeout, wsReadDeadline time.Duration) (Client, error) {
	log.Printf("Connecting to %v...", url)

	ctx, cancel := context.WithTimeout(context.Background(), config.Default().DialTimeout)
	defer cancel()

	c, err := gethrpc.DialContext(ctx, url, insecureSkipVerify, headers, connectionTimeout, wsReadDeadline)
	if err != nil {
		return nil, err
	}
	cc := client{*c, url}
